...
class FlaxUpsample2D(nnx.Module):
...
class FlaxDownsample2D(nnx.Module):
...
class FlaxResnetBlock2D(nnx.Module):
    ...
    def __init__(
        ...
        self.norm1 = nnx.GroupNorm(
            in_channels, num_groups=groups, epsilon=1e-6, rngs=rngs
        )
        self.conv1 = nnx.Conv(
            in_channels,
            out_channels,
        ...
        self.norm2 = nnx.GroupNorm(
            out_channels, num_groups=groups, epsilon=1e-6, rngs=rngs
        )
    ...
    def __call__(self, hidden_states, deterministic=True):
        ...
        hidden_states = nnx.swish(hidden_states)
...
class FlaxAttentionBlock(nnx.Module):
    ...
    def __init__(
        ...
        dense = partial(nnx.Linear, channels, channels, dtype=dtype, rngs=rngs)
